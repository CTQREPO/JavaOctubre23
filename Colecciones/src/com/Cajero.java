...
////////////////////
	...
	public Ticket transferir(Cuenta origen, Cuenta destino, double monto) {
		...
		List<Object> datos = new ArrayList<Object>(); //lista para imprimir ticket
		
		Ticket ticket = null;/// variable en null para usarla despues
		
		if(cuentas.contains(origen)) { ///VERIFICA SI EXISTEN LAS CUENTAS ORIGEN Y DESTINO
			if(cuentas.contains(destino)) {		
			if(origen.getSaldoDisponible()<monto) {
				System.out.println("FONDOS INSUFICIENTES");
			}else if ((origen.getSaldoDisponible()-monto)<origen.getSALDO_MIN()) {
				System.out.println("el retiro dejaria por debajo del minimo permitido");
			}else if((destino.getSaldoDisponible()+monto)>destino.getSALDO_MAX()){     ///LA TRANSFERENCIA REVASA LA CANTIDAD MAXIMA DE LA CUENTA DESTINO
				System.out.println("el monto supera el limite de cuenta");
			}else {
				origen.setSaldoDisponible(origen.getSaldoDisponible()-monto);   ////CANTIDAD NUEVA EN ORIGEN DESPUES DE TRANSFERENCIA
				destino.setSaldoDisponible(destino.getSaldoDisponible()+monto); ///CANTIDAD NUEVA EN DESTINO DESPUES DE TRANSFRENCIA
				ticket = new Ticket(new Date(),origen.getSaldoDisponible(), "transferencia",1,folio+1,monto);
